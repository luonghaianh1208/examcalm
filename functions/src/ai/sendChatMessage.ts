...
/**
 * TRẠNG THÁI (01/09/2026): callable này HIỆN KHÔNG CÓ giao diện nào gọi tới.
 *
 * Màn Trò chuyện đã chuyển sang bề mặt "Hỏi về web app" (askWebAppHelp.ts)
 * theo Brand Guideline §6.2 và quyết định 1c của chủ sản phẩm: trợ giúp cách
 * dùng sản phẩm, không phải bạn tâm sự.
 *
 * Giữ lại chứ không xoá vì đây là 956 dòng đã qua bốn vòng review an toàn, và
 * chatbot đồng hành cảm xúc vẫn có thể quay lại như một tính năng riêng. Nếu
 * quyết định bỏ hẳn, xoá kèm: chatSessions/chatMessages trong firestore.rules,
 * deleteUserData, và các trường chat trong aiConfig.
 */
...
import { findRecentUnhandledAlert, upgradeCrisisAlert, writeCrisisAlert } from "./crisisAlerts";
...
// findRecentUnhandledAlert / isMoreSevere / writeCrisisAlert / upgradeCrisisAlert
// đã chuyển sang ./crisisAlerts để bề mặt "Hỏi về web app" dùng chung đúng
// logic chống lụt này. Chuyển nguyên văn, không đổi hành vi.
