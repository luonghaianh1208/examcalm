...
// mọi môi trường), providerLabel rỗng, hoặc enabled=false. Cả ba đều là
// trạng thái "chưa cấu hình xong" bình thường, không phải lỗi.
const UNAVAILABLE: AiPublicConfig = { providerLabel: "", enabled: false };
...
export async function getAiPublicConfig(): Promise<AiPublicConfig> {
  ...
  try {
    ...
    return { providerLabel, enabled: true };
  } catch {
